...
export default function GantiSandi(props) {
  ...
  const checkCurrentPassword = async (e) => {
    let params = [];
    params.push({ base_url: formValidation.base_url, id_nakes: dataLogin.id_nakes, old_pass: e.nativeEvent.text });
  ...
  const handleSubmit = async () => {
    ...
    if(val.status === false) {
      ...
      axios
      .post(formValidation.base_url + "nakes/updatePassword/" + dataLogin.id_nakes, formData)
      .then(() => {
        setLoading(false);
